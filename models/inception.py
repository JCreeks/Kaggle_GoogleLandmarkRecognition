...
def first_phase(trained=True, printGap=True, first_phase_train_reps=first_phase_train_reps):
    ...
    if not trained:
        ...
        inception_model.compile(optimizer=Adam(lr=0.0001), loss='categorical_crossentropy', metrics=['acc'])
    else:
        inception_model = load_model(data_folder + '1st_phase_inception_model.h5')
        
    if not os.path.exists(first_phase_folder):
        os.makedirs(first_phase_folder)
        
    for i in range(first_phase_train_reps):
        history = inception_model.fit_generator(train_img_class_gen,
        ...
        if i>=5:
            ...
            inception_model.save(first_phase_folder + str(ts) + '_inception_model.h5')
            save_obj(history.history, str(ts) + '_inception_history', folder=first_phase_folder)
        
        if printGap:
            steps = len(val_names_list)/batch_size
            predicts = inception_model.predict_generator(val_img_class_gen, steps=steps/10, verbose=2)##########
        ...
        inception_model.save(data_folder + '1st_phase_inception_model.h5')
...
def inception(trained=False, third_phase_train_reps=third_phase_train_reps):
    ...
    if not trained:
        ...
        new_inception_model = Model(inputs=inception_model.input, outputs=predictors)

        optimizer = Adam(lr=0.1234)
        
        new_inception_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    else:
        new_inception_model = load_model(data_folder + '3rd_phase_inception_model.h5') 
        optimizer = Adam(lr=0.1234)
        new_inception_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    ...
    for i in range(third_phase_train_reps):
        ...
        print(i, 'out of ', third_phase_train_reps, '\nlearning rate ', K.eval(new_inception_model.optimizer.lr))
...
#                                                    workers=4, callbacks=[LosswiseKerasCallback(tag='keras inception model')])
        ...
        if i>=5:
            ts = calendar.timegm(time.gmtime())
            new_inception_model.save(third_phase_folder + str(ts) + '_inception_model.h5')
            save_obj(history.history, str(ts) + '_inception_history.h5', folder=third_phase_folder)
...
def third_phase(trained=False, third_phase_train_reps=third_phase_train_reps):
    ...
    if not trained:
        ...
        inception_model = load_model(data_folder + '3rd_phase_inception_model.h5')
