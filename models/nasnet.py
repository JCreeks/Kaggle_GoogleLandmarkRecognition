...
def nasnet(trained=False, third_phase_train_reps=third_phase_train_reps):
    global nasnet_model, new_nasnet_model, optimizer
    ...
    if not trained:
        ...
        predictions = nasnet_model.layers[-1]
        dropout = Dropout(0.2)
        fc = nasnet_model.layers[-2]
        fc.kernel_regularizer = regularizers.l2(0.001)
        fc.activity_regularizer = regularizers.l1(0.001)

        x = dropout(fc.output)
        predictors = predictions(x)
        new_nasnet_model = Model(inputs=nasnet_model.input, outputs=predictors)

        optimizer = Adam(lr=0.1234)
        
        new_nasnet_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    else:
        new_nasnet_model = load_model(data_folder + '3rd_phase_nasnet_model.h5') 
        optimizer = Adam(lr=0.1234)
        new_nasnet_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    ...
    for i in range(third_phase_train_reps):
        ...
        K.set_value(new_nasnet_model.optimizer.lr, lr)
        print(i, 'out of ', third_phase_train_reps, '\nlearning rate ', K.eval(new_nasnet_model.optimizer.lr))
        history = new_nasnet_model.fit_generator(train_img_class_gen,
                                               steps_per_epoch=steps_per_small_epoch,
...
#                                                    epochs=small_epochs, verbose=2,
#                                                    validation_data=val_img_class_gen, validation_steps=val_steps_per_small_epoch,
#                                                    workers=4, callbacks=[LosswiseKerasCallback(tag='keras nasnet model')])
        print("iteration",i)
        if i % saves_per_epoch == 0:
            print('{} epoch completed'.format(int(i / saves_per_epoch)))
        ...
        if i>=50:
            ts = calendar.timegm(time.gmtime())
            new_nasnet_model.save(third_phase_folder + str(ts) + '_nasnet_model.h5')
            save_obj(history.history, str(ts) + '_nasnet_history.h5', folder=third_phase_folder)

    new_nasnet_model.save(data_folder + '3rd_phase_nasnet_model.h5')
...
def third_phase(trained=False, third_phase_train_reps=third_phase_train_reps):
    ...
    if not trained:
        ...
        nasnet_model = load_model(data_folder + '3rd_phase_nasnet_model.h5')

#     # add regularizers to the convolutional layers
#     trainable_layers_ratio = 1 / 2.0
#     trainable_layers_index = int(len(nasnet_model.layers) * (1 - trainable_layers_ratio))
#     for layer in nasnet_model.layers[:trainable_layers_index]:
...
#     second_phase()
    third_phase(trained=False, third_phase_train_reps=15)
