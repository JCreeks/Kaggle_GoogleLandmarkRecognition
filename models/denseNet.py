...
def first_phase(trained=True, printGap=True, first_phase_train_reps=first_phase_train_reps):
    ...
    if not trained:
        ...
        denseNet_model.compile(optimizer=Adam(lr=0.0001), loss='categorical_crossentropy', metrics=['acc'])
    else:
        denseNet_model = load_model(data_folder + '1st_phase_denseNet_model.h5')
        
    if not os.path.exists(first_phase_folder):
        os.makedirs(first_phase_folder)
        
    for i in range(first_phase_train_reps):
        history = denseNet_model.fit_generator(train_img_class_gen,
        ...
        if i>=5:
            ...
            denseNet_model.save(first_phase_folder + str(ts) + '_denseNet_model.h5')
            save_obj(history.history, str(ts) + '_denseNet_history', folder=first_phase_folder)
        
        if printGap:
            steps = len(val_names_list)/batch_size
            predicts = denseNet_model.predict_generator(val_img_class_gen, steps=steps/10, verbose=2)##########
        ...
        denseNet_model.save(data_folder + '1st_phase_denseNet_model.h5')
    
def denseNet(trained=False, third_phase_train_reps=third_phase_train_reps):
    global denseNet_model, new_denseNet_model, optimizer
    ...
    if not trained:
        ...
        predictions = denseNet_model.layers[-1]
        dropout = Dropout(0.2)
        fc = denseNet_model.layers[-2]
        fc.kernel_regularizer = regularizers.l2(0.001)
        fc.activity_regularizer = regularizers.l1(0.001)

        x = dropout(fc.output)
        predictors = predictions(x)
        new_denseNet_model = Model(inputs=denseNet_model.input, outputs=predictors)

        optimizer = Adam(lr=0.1234)
        
        new_denseNet_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    else:
        new_denseNet_model = load_model(data_folder + '3rd_phase_denseNet_model.h5') 
        optimizer = Adam(lr=0.1234)
        new_denseNet_model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['acc'])
    ...
    for i in range(third_phase_train_reps):
        ...
        K.set_value(new_denseNet_model.optimizer.lr, lr)
        print(i, 'out of ', third_phase_train_reps, '\nlearning rate ', K.eval(new_denseNet_model.optimizer.lr))
        history = new_denseNet_model.fit_generator(train_img_class_gen,
                                               steps_per_epoch=steps_per_small_epoch,
...
#                                                    epochs=small_epochs, verbose=2,
#                                                    validation_data=val_img_class_gen, validation_steps=val_steps_per_small_epoch,
#                                                    workers=4, callbacks=[LosswiseKerasCallback(tag='keras denseNet model')])
        print("iteration",i)
        if i % saves_per_epoch == 0:
            print('{} epoch completed'.format(int(i / saves_per_epoch)))
        ...
        if i>=5:
            ts = calendar.timegm(time.gmtime())
            new_denseNet_model.save(third_phase_folder + str(ts) + '_denseNet_model.h5')
            save_obj(history.history, str(ts) + '_denseNet_history.h5', folder=third_phase_folder)

    new_denseNet_model.save(data_folder + '3rd_phase_denseNet_model.h5')
...
def third_phase(trained=False, third_phase_train_reps=third_phase_train_reps):
    ...
    if not trained:
        ...
        denseNet_model = load_model(data_folder + '3rd_phase_denseNet_model.h5')

#     # add regularizers to the convolutional layers
#     trainable_layers_ratio = 1 / 2.0
#     trainable_layers_index = int(len(denseNet_model.layers) * (1 - trainable_layers_ratio))
#     for layer in denseNet_model.layers[:trainable_layers_index]:
